            ...
            <div class="navbar-inner">
                <div class="container-fluid">
                    <a class="brand" href="http://www.papaok.org"><i
                            class="icon-home icon-white"></i>灌水大乐园 <span class="sml_t">2.0</span></a>
...
<script type="text/javascript">
    $(function() {
        var username = $.cookie('useru'); // 获得cookie
        var password = $.cookie('userp');
    ...
    function add() {
        ...
        if($("#postrootaction").val()==='reply') {//回帖使用异步，否则不能定位到model
            $.ajax({
                type: "post",
                url: "article",
                data: "action=addc&title=" + $('#title').val() + "&content=" + txt + "&rootid=" + $("#postrootid").val() + "&userid=" + $('#userid').val() + "&id=" + $('#postrootid').val(),
                dataType: "text json",
                success: function (data) {
                    ...
                    $('#rshow').modal('show');
                    $('#post').modal('hide');
                    showhuitie($("#postrootid").val(), $('#userid').val(), $('#duid').val(), data);
                }
...
//    });
    ...
    */
    ...
    function judge() {
        ...
        $.ajax({
            data:"action=judge"+"&reusername="+$("#u_fname").val(),
            datatype:"text",
            url:"user",
            type:"post",
            statusCode:{
